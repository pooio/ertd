...
import com.metaShare.modules.bpm.graph.Edge;
import com.metaShare.modules.bpm.graph.Graph;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;

/*import com.api6.plate.core.util.SpringUtils;
import com.api6.plate.prototype.bpm.BpmConstants;
import com.api6.plate.prototype.bpm.graph.ActivitiHistoryGraphBuilder;
import com.api6.plate.prototype.bpm.graph.Edge;
import com.api6.plate.prototype.bpm.graph.Graph;*/
...
public class CustomProcessDiagramGenerator{
    ...
    private static Color RUNNING_COLOR = new Color(240,173,78);//黄色
    private static Color HISTORY_COLOR = new Color(92,184,92);//绿色
    private static Color SKIP_COLOR = Color.GRAY;//灰色
    private static Color NOT_PASS = new Color(212,63,58);//红色
                ...
                if (historicActivityInstance.getEndTime() == null) {
                   ...
                   /* String deleteReason = null;
                	...
                	if("endEvent".equals(historicActivityInstance.getActivityType())){
                		String endType = BpmConstants.PASS;
                		BpmUserTaskService bpmUserTaskService = SpringUtils.getBean("BpmUserTaskService");
                        BpmUserTask bpmUserTask = bpmUserTaskService.selectByTaskId(historicActivityInstance.getProcessInstanceId());
                		/*HistoryService historyService = SpringUtils.getBean("historyService");
                		endType = (String) historyService.createHistoricProcessInstanceQuery()
                				.processInstanceId(historicActivityInstance.getProcessInstanceId())
                				.includeProcessVariables()
                		.singleResult().getProcessVariables().get(BpmConstants.PROCESS_END_TYPE);*/
                		if(bpmUserTask.getApprovalStatus() == 3){
                		    endType = BpmConstants.NOT_PASS;
                        }
                       ...
                       try {
                           drawNodeBorder(activity.getX() - this.minX, 
                        		   activity.getY() - this.minY, activity.getWidth(), 
                        		   activity.getHeight(), graphics, nodeColor,
                        		   historicActivityInstance.getActivityType());
                       } finally {
                           graphics.dispose();
                       }
    ...
    public void drawSequenceFlow(BufferedImage image,
        ...
        try {
        	if(StringUtils.isEmpty(sequenceFlowId)){
        		return;
        	}
            List<GraphicInfo> graphicInfoList = bpmnModel
