    ...
    <div class="box">
        ...
        <el-row :gutter="10">
            ...
            <el-col :span="20">
                ...
                <el-button type="primary" title="新增轮播图" icon="el-icon-plus" style="margin-left:20px" @click="$refs.add.openDialog(addInfo,moduleList)" v-has="'system-config-add'" id="system-config-add">新增</el-button>
            </el-col>
        </el-row>
        <el-table :data="tableData" style="width: 100%" height="calc(100% - 110px)" border>
            ...
            <el-table-column prop="configContent" label="图片" :resizable="false">
                <template slot-scope="scope">
                    <el-image  style="width: 10
                    0px; height: 100px" :src="scope.row.img" fit="fill"></el-image>
                </template>
            </el-table-column>
...
export default {
    ...
    methods: {
         ...
         */
        async getTableData() {
            const res = await this.$axios.post('sys/banner/getList', this.searchInfo);
            if (res.data.code == 0) {
        ...
        handleToDel(id) {
            this.$confirm('确定删除吗？', '提示', {
                ...
                .then(async () => {
                    const res = await this.$axios.post('sys/banner/del', { id });
                    if (res.data.code == 0) {
