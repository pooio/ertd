    ...
    <div class="right-board">
        ...
        <i class="el-icon-link" />
            ...
            <el-scrollbar class="right-scrollbar">
                <!-- 组件属性 -->
                <el-form v-if="this.activeData.__config__!=null" v-show="currentTab==='field' && showField" size="small" label-width="90px">
            ...
            <el-input v-model="activeData.name" placeholder="请输入上传文件字段名" />
                    ...
                    <el-form-item v-if="activeData.action !== undefined" label="上传地址">
                        <el-input v-model="activeData.action" placeholder="请输入上传地址" disabled />
                    </el-form-item>
                    ...
                    <!-- <el-form-item v-if="['el-select'].indexOf(activeData.__config__.tag) > -1 && funcListShow" label="数据模式"> -->
                    <el-form-item v-if="['el-select'].indexOf(activeData.__config__.tag) > -1" label="数据模式">
                        <el-radio-group v-model="activeData.dataType" @change="changeDataType">
                            <el-radio-button label="1">数据字典</el-radio-button>
                            <el-radio-button label="2">业务数据</el-radio-button>
                        </el-radio-group>
                    </el-form-item>

                    <!-- 点选、多选、选择器 -->
                    <el-form-item v-if="['el-checkbox-group', 'el-radio-group', 'el-select'].indexOf(activeData.__config__.tag) > -1 && funcListShow && activeData.dataType == 1" label="远程绑定">
                        <el-select v-model="activeData.__config__.func" placeholder="请选择绑定数据" :style="{width: '100%'}" @change="funcChange">
                            <el-option v-for="item in funcList" :key="item.value" :value="item.value" :label="item.label"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item v-if="['el-checkbox-group', 'el-radio-group', 'el-select'].indexOf(activeData.__config__.tag) > -1 && funcListShow && activeData.dataType == 2" label="远程绑定">
                        <el-select v-model="activeData.__config__.func" placeholder="请选择绑定数据" :style="{width: '100%'}" @change="funcChange">
                            <el-option v-for="item in funcList2" :key="item.value" :value="item.value" :label="item.label"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item v-if="activeData.dataType == 2" label="业务字段">
                        <el-input v-model="activeData['dataTypeName']" placeholder="请输入业务字段" @blur="getDataDictList" />
                    </el-form-item>
                    <!-- 级联选择器 -->
                    <el-form-item v-if="['el-cascader'].indexOf(activeData.__config__.tag) > -1" label="远程绑定">
                        <el-select v-model="activeData.__config__.cascader" placeholder="请选择绑定数据" :style="{width: '100%'}" @change="cascaderChange">
                            <el-option v-for="item in cascaderList" :key="item.value" :value="item.value" :label="item.label"></el-option>
                        </el-select>
                    </el-form-item>
                    <template v-if="['el-checkbox-group', 'el-radio-group', 'el-select'].indexOf(activeData.__config__.tag) > -1 && activeData.__config__.func==''">
                  ...
                  静态数据
                        ...
                        <template v-if="activeData.__config__.dataType === 'dynamic'">
                            <el-form-item label="标签键名">
                                <el-input v-model="activeData.props.props.label" placeholder="请输入标签键名" disabled />
                            </el-form-item>
                            <el-form-item label="值键名">
                                <el-input v-model="activeData.props.props.value" placeholder="请输入值键名" disabled />
                            </el-form-item>
                            <el-form-item label="子级键名">
                                <el-input v-model="activeData.props.props.children" placeholder="请输入子级键名" disabled />
                            </el-form-item>
                        </template>
                    ...
                    <el-form-item v-if="activeData.__config__.showStatistic !== undefined
            && activeData.__config__.labelWidth !== undefined" label="列表展示">
                        <el-switch v-model="activeData.__config__.showStatistic" />
                    </el-form-item>
            ...
            <el-switch v-model="activeData['auto-upload']" />
                    ...
                    <el-form-item v-if="activeData.__config__.tag != 'el-color-picker' && activeData.__config__.tag != 'el-upload' && activeData.__config__.required !== undefined" label="是否必填">
                        <el-switch v-model="activeData.__config__.required" />
                    </el-form-item>
                    <el-form-item v-if="activeData.__config__.tag === 'el-input'" label="当前用户">
                        <el-switch v-model="activeData.isCurrentUser" @change="isCurrentUserChange" />
                    </el-form-item>
...
import TreeNodeDialog from '@/views/customerForm/TreeNodeDialog';
import { isNumberStr } from '@/utils/index';
import IconsDialog from './IconsDialog';
import { inputComponents, selectComponents, layoutComponents } from '@/components/generator/config';
import { saveFormConf } from '@/utils/db';
import { getTreeNode, getUserList, getDictList, getDeptList, getDataList } from '@/utils/funcData';
...
export default {
    ...
    props: ['showField', 'activeData', 'formConf', 'parentId'],
    data() {
        return {
            currentTab: 'field',
            currentNode: null,
    ...
    created() {
        this.activeData.__config__.func = this.parentId;
    },
    mounted() {
        this.getFuncList();
    },
    ...
    methods: {
        ...
        onDefaultValueInput(str) {
            if (this.activeData.__config__.tag == 'el-button') {
                if (str.length > 10) {
                    this.$message.error('按钮默认值不能超过10个字');
        ...
        async getFuncList() {
            this.funcList2 = await getDataList();
            // this.activeData.__config__.func = this.parentId;
            this.funcList = await getTreeNode();
            ...
            this.funcListShow = true;
        },
        async funcChange(val) {
            this.activeData.businessId = val;
            this.activeData.__config__.renderKey = +new Date();
            if (this.activeData.__config__.tag == 'el-checkbox-group') {
            ...
            if (val == 'user') {
                this.activeData.__slot__.options = await getUserList();
            } else {
                this.activeData.__slot__.options = await getDictList(val);
            }
        },
        async cascaderChange(val) {
            ...
            this.$set(this.activeData.__config__, 'defaultValue', []);
            if (val == '') {
                this.activeData.__config__.dataType = 'static';
            ...
            if (val == 'dept') {
                ...
                this.activeData.options = await getDeptList();
            }
        },
        isCurrentUserChange(val) {
            this.activeData.disabled = val;
        },
        // 数据模式
        changeDataType(val) {
            this.funcList = [];
            this.activeData.__config__.func = null;
            if (val == 2) {
                this.activeData.dataTypeName = '';
                this.dataTypeNameBlur();
            } else {
                this.getFuncList();
            }
        },
        async dataTypeNameBlur() {
            // if (this.activeData.dataTypeName == '') {
            //     this.funcList = [];
            // } else {
            this.funcList = await getDataList();
            // }
        },
        async getDataDictList() {
            var arr = [];
            const res = await this.$axios.post('formData/getCustomDirData', {
                formId: this.activeData.businessId,
                column: this.activeData['dataTypeName']
            });
            ...
            // return arr;
            this.activeData.__slot__.options = arr;
        }
    }
};
</script>
